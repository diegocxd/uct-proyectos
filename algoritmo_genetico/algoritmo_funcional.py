...
def mutar(individuo, tasa_mutacion=0.1): 
    ...
    individuo.probs = np.clip(individuo.probs, 0, None) 
    individuo.probs = individuo.probs / np.sum(individuo.probs) 
...
def ejecutar_generacion(individuos): 
    grida = np.full((grida_dim, grida_dim, 3), 255) #para limpiar
    posiciones_ocupadas = set((ind.y, ind.x) for ind in individuos)  #crea un conjunto 
    individuos_en_meta = []
    primer_en_meta = None
...
for i in range(num_generaciones):
    ...
    if len(individuos_en_meta) > 0: 
        ...
        pasos_primer_en_meta_generaciones.append(primer_en_meta.pasos)  #para la grafica
        probabilidades1 = probabilidades(individuos_en_meta)
        for i, ind in enumerate(individuos_en_meta): 
            print(f"Individuo {i+1}: Fitness: {ind.fitness()}, Probabilidad de ser seleccionado: {probabilidades1[i]}, semilla utlizada: {seed}")
        nuevos_individuos = []
...
plt.figure()
plt.plot(generaciones, pasos_promedio_generaciones)
...
plt.figure()
plt.plot(generaciones, pasos_primer_en_meta_generaciones)
